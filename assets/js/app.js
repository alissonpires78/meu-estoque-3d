/* === app.js | PARTE 1/4 | cole primeiro === */
...
function filterFilaments() {
  ...
  filteredFilaments = filaments.filter(f => {
    ...
    const matchBrand = !brand || (f.marca || '').trim() === brand;
    const matchStatus = !status || f.status === status;
    const matchMaterial = !material || f.material === material;

    return matchSearch && matchColor && matchBrand && matchStatus && matchMaterial;
  });
...
/* === app.js | PARTE 2/4 | cole após a PARTE 1/4 === */
...
/* === app.js | PARTE 3/4 | cole após a PARTE 2/4 === */
...
/* === app.js | PARTE 4/4 | cole após a PARTE 3/4 === */
...
function exportJSON() {
  ...
  downloadFile(blob, "estoque_3d_backup.json");
  showToast("Exportado (filamentos + impressoras + histórico)!");
}
...
function exportCSV() {
  ...
  const rows = filaments.map(f => [
    ...
    f.preco_pago,
    f.gramas,
    (f.metros || 0).toFixed(0),
    (f.localizacao || '').replaceAll(',', ' '),
    f.status
  ]);
...
function handleImport(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      // Aceita:
      // 1) array direto -> filamentos
      // 2) objeto { filaments, printers, usageHistory } -> pacote completo
